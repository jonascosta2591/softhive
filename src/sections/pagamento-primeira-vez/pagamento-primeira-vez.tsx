...
const PagamentoPrimeiraVez: React.FC = () => {
  ...
  const [textButtonCopiaEcola, setTextButtonCopiaECola] = useState<string>(
    ...
    if (method === 'pix') {
      ...
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_API_URL}/comprar_software/comprar`,
          {
            softwaresIds: IdsSoftwaresEscolhidos,
            paymentMethod: 'PIX',
            paymentUserData: [{ name: firstName + ' ' + lastName, cpfCnpj: cpf }],
          },
          {
            headers: {
          ...
          setInterval(async () => {
            try {
              const responseVerifyTransaction = await axios.get(
                `${import.meta.env.VITE_API_URL}/payment/payment?paymentId=${response.data.idTransaction}`,
                {
                  headers: {
                    Authorization: `Bearer ${AuthorizationToken}`,
                  },
  ...
  const validate = () => {
    ...
    if (method === 'credit') {
      ...
      if (phoneDigits.length < 10) {
        console.error('Por favor, insira um telefone válido.');
        return false;
      }
      ...
      if (!address.trim()) {
        console.error('Por favor, insira o endereço.');
        return false;
      }
                      ...
                      <Box sx={{ mb: 4, textAlign: 'center', p: 4 }}>
                        ...
                        {imagemPixQrCode && (
                          <>
                            ...
                            <Button
                              onClick={HandleCopiarCopiaEcolaPix}
                              sx={{
                                backgroundColor: '#00C2E6',
                                color: '#000',
                              }}
                            >
                              {textButtonCopiaEcola}
                            </Button>
...
export default PagamentoPrimeiraVez;
export { PagamentoPrimeiraVez };